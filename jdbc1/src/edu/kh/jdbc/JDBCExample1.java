...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// java.sql.Statement (sql에 있는거임 빈즈로 하나 더있는거 낚이지말기)
		// - 1) SQL을 Java -> DB에 전달
		// - 2) DB에서 SQL 수행 결과를 반환 받아옴 (DB->Java)
		Statement stmt = null;
		
		//DB에서 셀렉트 결과가 테이블처럼 나오는 그거
		//java.sql.ResultSet
		// - SELECT 조회 결과를 저장하는 객체
		// - UPDATE,INSERT,DELETE엔 필요없음 그떈 int 정도만 있어도 됨
		ResultSet rs = null;
		...
		try {
			...
			// ㅋㅋㅋㅋㅋ 이름이 Class인 클래스임(스태틱 메서드라 이렇게 불러오기 가능)
			// Class.forName("패키지명(oracle.jdbc.driver).클래스명");
			// - 해당 클래스를 읽어 메모리에 적재
			...
			// 위에거 다합쳐보면 "jdbc:oracle:thin:@localhost:1521:XE"
			// DB에서 계정명위에 마우스 올려보면 똑같이 나옴
			...
			String sql = "SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY, HIRE_DATE "
					+ "FROM EMPLOYEE";
			// +로 연결해도되는데 공백 포함인지 잘 확인하기
			
			/* 4.Statement 객체 생성 */
			// sql를 실어가줄 버스 (커넥션이 만들어줌)
			...
			// 결과로 int 반환(삽입,수정,삭제된 행의 개수 반환)
			
			rs = stmt.executeQuery(sql);
			
			/* 6. 조회 결과가 담겨있는 ResultSet을
			 * 커서(Cursor) 1행 씩 접근해 각행에 작성된 컬럼 값 얻어오기
			 * 
			 */
			...
			//	문제가 발생할 수 있다!!!!
			// ** DB에 뚫는 Connection은 갯수 한계가 있기에 그걸 방지하고 하는 것
